...
export const Carousel = ({ items, initialScroll = 0 }: CarouselProps) => {
  ...
  const handleCardClose = (index: number) => {
    if (carouselRef.current) {
      const cardWidth = isMobile() ? 280 : 320; // Updated for new dimensions
  ...
  return (
    <>
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="rounded-3xl bg-gray-100 dark:bg-neutral-900 h-[600px] w-[280px] md:h-[700px] md:w-[320px] overflow-hidden flex flex-col items-start justify-start relative z-10"
      >
        <BlurImage
          src={card.src}
          alt={card.title}
